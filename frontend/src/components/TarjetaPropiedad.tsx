...
const TarjetaPropiedad = ({ propiedad }: { propiedad: Propiedad }) => {
  ...
  return (
    <Link to={`/propiedades/${propiedad.id}`} style={{ textDecoration: 'none' }}>
      <div style={{
        ...
        onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-4px)'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 12px 40px rgba(27,58,107,0.15)'; }}
        onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(0)'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 2px 16px rgba(27,58,107,0.08)'; }}
      >
        {/* Imagen */}
        <div style={{ position: 'relative', height: 220, overflow: 'hidden', background: '#e8ecf0' }}>
          {imagen ? (
            <img src={imagen} alt={propiedad.titulo} style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.3s' }} />
          ) : (
          ...
          <div style={{ position: 'absolute', top: 12, left: 12, display: 'flex', gap: 6 }}>
            <span style={{ background: propiedad.operacion === 'arriendo' ? '#1B3A6B' : '#059669', color: 'white', padding: '4px 10px', borderRadius: 20, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
        ...
        <div style={{ padding: '20px' }}>
          <h3 style={{ fontSize: 16, fontWeight: 700, color: '#111827', marginBottom: 8, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {propiedad.titulo}
          </h3>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#6b7280', fontSize: 13, marginBottom: 16 }}>
            <MapPin size={12} />
            <span>{propiedad.localidad}{propiedad.barrio ? `, ${propiedad.barrio}` : ''}</span>
          </div>
